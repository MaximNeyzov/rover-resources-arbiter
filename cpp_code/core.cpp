...
public:
   iOut iter(Mes mes, bool pi, bool pj, bool highPrior) {
      ...
      print_input(mes, pi, pj, highPrior);   // for debug.
      print_result(q, iout, ind, imes);  // for debug.
      return iout;
   }

// for debug:
private:
   std::string getStateName(Q q) {
      switch (q) {
...
// for debug:
private:
   std::string getOutName(iOut iout) {
      switch (iout) {
...
// for debug:
private:
   std::string getIMesName(iMes imes) {
      switch (imes) {
...
private:
   void print_input(Mes mes, bool pi, bool pj, bool highPrior) {
      std::cout << "INPUT:  ";
      if (mes == REQ)  { std::cout << "[mes==req]  ";  }
      if (mes == FREE) { std::cout << "[mes==free]  "; }
      std::cout << "[pi==" << pi << "]  ";
      std::cout << "[pj==" << pj << "]  ";
      std::cout << "[highPrior==" << highPrior << "]" << std::endl;
   }

// for debug:
private:
   void print_result(Q q, iOut iout, I ind, iMes imes) {
      std::cout << "RESULT:  ";
      std::cout << "[q==" << getStateName(q) << "]  ";
      std::cout << "[iout==" << getOutName(iout) << "]  ";
      std::cout << "[ind==" << getIndexName(ind) << "]  ";
      std::cout << "[imes==" << getIMesName(imes) << "]" << std::endl;
   }
};